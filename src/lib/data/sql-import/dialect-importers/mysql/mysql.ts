...
/**
 * dbdiagram / Supabase-style exports often use MySQL backticks but PostgreSQL
 * column types. Those must not be routed to the MySQL regex importer.
 */
function looksLikePostgresDDLWithMysqlStyleQuotes(sqlContent: string): boolean {
    const lower = sqlContent.toLowerCase();
    return (
        /\bjsonb\b/.test(lower) ||
        /\bfloat8\b/.test(lower) ||
        /\bint8\b/.test(lower) ||
        /\benum\s*\[/.test(lower) ||
        /\btext\s*\[/.test(lower)
    );
...
export function isMySQLFormat(sqlContent: string): boolean {
    // Minimal MySQL-flavored DDL (e.g. dbdiagram / tool exports): backtick identifiers, no ENGINE= lines
    if (/CREATE\s+TABLE\s+`/i.test(sqlContent)) {
        if (looksLikePostgresDDLWithMysqlStyleQuotes(sqlContent)) {
            return false;
        }
        return true;
    }
