...
/**
 * When the diagram is "Generic", pick a concrete SQL dialect for import/validation.
 */
export function resolveEffectiveSqlSourceType(
    sqlContent: string,
    ...
    return detectedType ?? DatabaseType.POSTGRESQL;
}
...
export async function sqlImportToDiagram({
    ...
    const effectiveSourceType = resolveEffectiveSqlSourceType(
        sqlContent,
        sourceDatabaseType
    );
