...
export const DiagramFilterProvider: React.FC<React.PropsWithChildren> = ({
    ...
    useEffect(() => {
        ...
        const trimmedId = diagramId?.trim() ?? '';
        // ChartDB initializes diagramId as '' until loadDiagram completes; still run
        // the branch below so we never leave loading stuck true with no async to finish.
        if (!trimmedId) {
        ...
        const loadFilterFromStorage = async (id: string) => {
            try {
                const storedFilter = await getDiagramFilter(id);
        ...
        void loadFilterFromStorage(trimmedId);
